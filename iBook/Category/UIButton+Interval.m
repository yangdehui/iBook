...
//  University
//
//  Created by zikeys on 2018/5/22.
//  Copyright © 2018年 知企网. All rights reserved.
//
...
- (void)SG_sendAction:(SEL)action to:(id)target forEvent:(UIEvent *)event {
    if (self.SG_eventTimeInterval > 0) {
        ...
        SEL systemSEL = @selector(sendAction:to:forEvent:);
        SEL replaceSEL = @selector(SG_sendAction:to:forEvent:);
        Method systemMethod = class_getInstanceMethod([UIButton class], systemSEL);
        Method replaceMethod = class_getInstanceMethod([UIButton class], replaceSEL);
        method_exchangeImplementations(replaceMethod, systemMethod);
